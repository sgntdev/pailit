<script>
    ...
    let dataAsset = data.body.dataAsset;
	let assetSelling = {
		debitorId: dataAsset.debitorId,
		sellingPrice: formatPrice(dataAsset.sellingPrice)
	};
	let loading = false;
	...
	const handleUpdateAsset = async () => {
		loading = true;
		try {
			const response = await fetch(`/api/assetselling/${dataAsset.id}`, {
				method: 'PUT',
				headers: {
